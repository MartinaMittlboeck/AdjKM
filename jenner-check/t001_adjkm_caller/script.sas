/* AdjKM_SAS_Macro.sas by Martina Mittlboeck (Medical University of Vienna),  */
/* verbatim below, invoked on a small synthetic survival cohort so the        */
/* %AdjKM macro runs self-contained (no external library / data file needed). */
/* The 50 rows exercise all three donor branches the macro handles:           */
/* donor identified, no donor, and donor-search ceased.                       */

data work.simbsp;
  input binary_time time event;
  datalines;
0.22 6.94 1
0.58 3.58 1
4.43 7.25 1
...
/* invoke the macro on the synthetic cohort (positional args, matching the     */
/* %macro AdjKM(...) signature above); naive analysis, t*=5, tsearch=5.        */
%AdjKM(work.simbsp, binary_time, time, event, 5, 5, , 0.05, NO, 0, work.adjkm_out);
